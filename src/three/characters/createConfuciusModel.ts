...
export function createConfuciusSeatedBustModel(options: ProceduralModelOptions = {}): THREE.Group {
  ...
  materialMap["skin"] = createSculptMaterial(
    "skin",
    {"id": "skin", "kind": "MeshStandardMaterial", "roughness": 0.65, "metalness": 0.0, "localOverrides": ["head.faceWrinkles"], "notes": "Warm-toned aged skin, low-mid roughness, no true SSS; slightly desaturated shadow tint per grimoire/character/reconstruction.md skin recipe.", "baseColor": "#c9986f"},
    options
  );
  materialMap["hair"] = createSculptMaterial(
    "hair",
    {"id": "hair", "kind": "MeshStandardMaterial", "roughness": 0.8, "metalness": 0.0, "localOverrides": ["beard.clumpSystem"], "notes": "Grey/white stylized hair clumps (beard, mustache, eyebrows, temple fringe) — no strand-level geometry, per single-image reconstruction guidance.", "baseColor": "#8f8a86"},
    options
  );
  materialMap["robeSilk"] = createSculptMaterial(
    "robeSilk",
    {"id": "robeSilk", "kind": "MeshPhysicalMaterial", "roughness": 0.42, "metalness": 0.0, "clearcoat": 0.15, "clearcoatRoughness": 0.3, "localOverrides": ["robeChest.dragonMedallionMask", "rightSleeve.drape"], "notes": "Rust-red silk, moderate sheen (clearcoat) reading as satin/brocade; gold dragon-medallion embroidery as an albedo + normal local override mask.", "baseColor": "#8a2f1f"},
    options
  );
  materialMap["collarSilk"] = createSculptMaterial(
    "collarSilk",
    {"id": "collarSilk", "kind": "MeshStandardMaterial", "roughness": 0.5, "metalness": 0.0, "localOverrides": ["collarPanel.embroideryMask"], "notes": "Black inner lapel panel; gold-thread dragon/cloud embroidery as a brighter albedo mask, thin gold piping edge.", "baseColor": "#1c1a1c"},
    options
  );
  materialMap["sashSilk"] = createSculptMaterial(
    "sashSilk",
    {"id": "sashSilk", "kind": "MeshStandardMaterial", "roughness": 0.48, "metalness": 0.0, "localOverrides": ["waistSash"], "notes": "Diagonal black waist sash, same embroidery-mask treatment as the collar panel, gold piping edge.", "baseColor": "#211d1f"},
    options
  );
  materialMap["cap"] = createSculptMaterial(
    "cap",
    {"id": "cap", "kind": "MeshStandardMaterial", "roughness": 0.55, "metalness": 0.0, "notes": "Red felt/silk scholar's cap body.", "baseColor": "#7a1f22"},
    options
  );
  materialMap["capTrim"] = createSculptMaterial(
    "capTrim",
    {"id": "capTrim", "kind": "MeshStandardMaterial", "roughness": 0.5, "metalness": 0.0, "notes": "Dark maroon trim band and ribbon ties.", "baseColor": "#4a1416"},
    options
  );
  materialMap["goldTrim"] = createSculptMaterial(
    "goldTrim",
    {"id": "goldTrim", "kind": "MeshPhysicalMaterial", "roughness": 0.3, "metalness": 0.85, "clearcoat": 0.2, "notes": "Metal-like gold trim/ornament/piping accents (cap button, collar+sash piping, embroidery thread highlights).", "baseColor": "#c9a341"},
    options
  );
  materialMap["eyeWhite"] = createSculptMaterial(
    "eyeWhite",
    {"id": "eyeWhite", "kind": "MeshStandardMaterial", "roughness": 0.15, "metalness": 0.0, "notes": "Glossy sclera base; iris disc + catchlight are a separate decal/emissive dot per character material recipe.", "baseColor": "#e8e2d8"},
    options
  );
  ...
  if (endpoint_arm_r_6) {
    ...
    node_arm_r_6.position.set(0.3, 0.12, 0.1);
